...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, patchsize, img_size, in_channels):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        self.position_embeddings = nn.Parameter(torch.zeros(1, n_patches, in_channels // 2))
...
class Attention_org(nn.Module):
    def __init__(self, channel_num,cfg):
        ...
        key = nn.Linear(self.KV_size, cfg[0], bias=False)
        value = nn.Linear(self.KV_size, cfg[0], bias=False)
        for _ in range(4):
    ...
    def forward(self, emb1,emb2,emb3,emb4, emb_all):
        ...
        O1 = self.out1(context_layer1) if emb1 is not None else None # 1024,14
...
class Block_ViT(nn.Module):
    def __init__(self, channel_num,cfg):
        ...
        channel_num_sum = channel_num[0] + channel_num[1] + channel_num[2] + channel_num[3]
        self.attn_norm =  LayerNorm(channel_num_sum,eps=1e-6)
        self.channel_attn = Attention_org(channel_num,cfg)
        ...
        self.ffn1 = Mlp(channel_num[0], cfg[1])
        self.ffn2 = Mlp(channel_num[1], cfg[2])
        self.ffn3 = Mlp(channel_num[2], cfg[3])
        self.ffn4 = Mlp(channel_num[3], cfg[4])
...
class Encoder(nn.Module):
    def __init__(self,  channel_num,cfg):
        ...
        for i in range(1):
            if cfg is not None:
                layer = Block_ViT(channel_num,cfg=cfg[i])
            else:
                layer = Block_ViT(channel_num, cfg=None)
...
class ChannelTransformer_slim(nn.Module):
    def __init__(self, img_size, channel_num, patchSize,cfg=None):
        super().__init__()
        ...
        new_channel_num = [item // 2 for item in channel_num]
        self.encoder = Encoder(new_channel_num,cfg=cfg)

        self.reconstruct_1 = Reconstruct(new_channel_num[0], channel_num[0], kernel_size=1,scale_factor=(self.patchSize_1,self.patchSize_1))
        self.reconstruct_2 = Reconstruct(new_channel_num[1], channel_num[1], kernel_size=1,scale_factor=(self.patchSize_2,self.patchSize_2))
        self.reconstruct_3 = Reconstruct(new_channel_num[2], channel_num[2], kernel_size=1,scale_factor=(self.patchSize_3,self.patchSize_3))
        self.reconstruct_4 = Reconstruct(new_channel_num[3], channel_num[3], kernel_size=1,scale_factor=(self.patchSize_4,self.patchSize_4))
